...
flowchart TD
  ...
  AuthorGuide["/getting-started-for-authors (author enablement)"] --> Drafts["/drafts (WIP landing)"]
  Drafts --> StarterPage["/drafts/starter-page (duplicable template)"]
```
`/getting-started-for-authors`, `/drafts`, and `/drafts/starter-page` are author-enablement
pages, not part of the visitor journey — they're excluded from `helix-query.yaml`'s indices
(both `products` and the general `pages` index) and from the sitemap, but they exist as real
seeded DA documents like any other page (see "Incident record #3" in
`../../docs/onboarding/da-content-onboarding-and-cors.md`).
...
## Sitemap
- Machine: `helix-sitemap.yaml` → `/sitemap.xml` (from the `products` index) and `/sitemap-pages.xml` (from the general `pages` index); both indices exclude `/drafts/**` and `/block-library/**`. `robots.txt` allows GPTBot/PerplexityBot/ClaudeBot (GEO/AEO).
